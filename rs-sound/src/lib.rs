...
use bevy::audio::{
    AudioPlayer, AudioSink, AudioSinkPlayback, AudioSource, DefaultSpatialScale, PlaybackSettings,
...
const DEFAULT_SOUND_EVENT: &str = "minecraft:gui.button.press";
...
impl Plugin for SoundPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<SoundSettings>()
            ...
            .add_systems(Startup, setup_sound_runtime)
            .add_systems(
                Update,
                (
                    ensure_spatial_listener,
                    drain_sound_events,
                    sync_playing_sound_volumes,
                ),
            );
    }
}
...
fn setup_sound_runtime(mut commands: Commands) {
